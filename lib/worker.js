...
function Worker(configOrConnection, event, options) {
    ...
    self._events['complete'] = function(message) {
        self.deleteMessage(message);    // delete message from queue
    };

    /*self._events['error'] = function(err) {
        self.logger.error("processing failure: %s", err.toString());
    };*/
