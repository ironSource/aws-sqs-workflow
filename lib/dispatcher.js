...
function Dispatcher(configOrConnection, options, events) {
    ...
    self.dispatch = function (event, message) {
        return self.getQueueUrl(event)
            ...
            .then(function (message) {
                self.logger.debug(util.format("dispatched new event: %s, message Id: %s", event, message.MessageId));
            })
            .catch(function (e) {
